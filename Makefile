.PHONY: fmt fmt-fix lint lint-fix doc docs docs-serve dev build check \
...
.PHONY: typecheck deny audit-stats
...
# Build the MkDocs site (the published documentation under docs/), strict so a
# broken link or nav entry fails the build. Output to site/.
docs:
...
# Feature matrix: wepwolf currently exposes no Cargo features, so this is a
...
# FR-* <-> test coverage and the single-accept-path invariant (C4, FR-TEST-2).
# The audit scripts are local dev tooling; skip cleanly when they are not present.
audit:
	@if [ -f scripts/audit_fr.sh ]; then bash scripts/audit_fr.sh; else echo "audit: scripts/ not present -- skipping FR<->test coverage check"; fi

# STATS.md banner contract: every Stats field documented, and vice versa.
audit-stats:
	@if [ -f scripts/audit_stats.sh ]; then bash scripts/audit_stats.sh; else echo "audit-stats: scripts/ not present -- skipping STATS.md contract check"; fi
...
#
# wepwolf has no platform-conditional dependencies, so every target builds
# with the same flags.
...
build-linux-musl:
	@if ! command -v musl-gcc > /dev/null 2>&1; then \
		...
		$(CARGO) build --profile release -p wepwolf --bin wepwolf --target $(TARGET_LINUX_X86_MUSL); \
	fi
...
build-linux-arm-musl:
	@if ! command -v musl-gcc > /dev/null 2>&1; then \
		...
		$(CARGO) build --profile release -p wepwolf --bin wepwolf --target $(TARGET_LINUX_ARM_MUSL); \
	fi
...
build-macos-arm:
	...
	$(CARGO) build --profile release -p wepwolf --bin wepwolf --target $(TARGET_MACOS_ARM)
...
build-macos-x86:
	...
	$(CARGO) build --profile release -p wepwolf --bin wepwolf --target $(TARGET_MACOS_X86)
...
# Primary gate: formatting, lints, types, tests, and the spec audits.
check: fmt lint typecheck test audit audit-stats

# Full verification gate -- run before every push (adds supply-chain, docs, hygiene).
check-all: fmt lint deny typecheck test-matrix audit audit-stats doc hygiene machete
...
cut-release:
	...
	cargo set-version -p wepwolf "$(VERSION)"
