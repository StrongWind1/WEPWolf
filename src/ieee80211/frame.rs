//! Phase 2 -- Decode: 802.11 MAC header + address mapping; malformed frames are counted and skipped, never fatal (FR-PARSE-2). See specs/02-architecture.md §3.2 + §8.3.
//!
...
pub struct MacHeader {
    ...
    pub sta: MacAddr,
    /// The L2 Destination Address (DA), derived from ToDS/FromDS per Table 9-60:
    /// `Addr3` when `ToDS` is set, else `Addr1`. This stays cleartext even when the
    /// payload is WEP-encrypted, so a multicast/broadcast DA reveals the L3
    /// protocol family (RFC 1112 §6.4, RFC 2464 §7) -- which `crate::classify` uses
    /// to pick the right known-plaintext without decrypting.
    pub dst: MacAddr,
...
/// log lines on a 70 M-packet capture. Treating non-zero Protocol Version frames as
/// hard malformed loses ~11.7 K frames per capture that real-world dissectors accept;
...
/// `[malformed_frame]` log entries on a 70 M-packet capture that were actually
...
pub fn parse(data: &[u8]) -> ParseResult {
    ...
    // L2 Destination Address per Table 9-60: Addr3 when `ToDS` is set (the frame is
    // relayed through the AP to its true destination), otherwise Addr1.
    let dst = if to_ds { addr3 } else { addr1 };
